...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
		...
		kthread_t * new_thread = (kthread_t *)slab_obj_alloc(kthread_allocator);	/*slab_obj_alloc = malloc*/

		new_thread->kt_kstack = alloc_stack(); /* the kernel stack */
...
kthread_cancel(kthread_t *kthr, void *retval)
{
		...
		if(kthr==curthr) kthread_exit(retval);	//cancel current thread
		else if(kthr->kt_state == KT_SLEEP_CANCELLABLE){ //the thread is cancellable
			sched_cancel(kthr);
			kthr->kt_cancelled = 1;
